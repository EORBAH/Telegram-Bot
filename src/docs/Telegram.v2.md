...
4. Utilisation
   ...
   - **Gestion des Fichiers (Nouveau)**
     ...
     - Télécharger des fichiers depuis Telegram
     - Télécharger des photos
     - Extraire les photos des messages
   - Actions de Chat
...
5. Exemples Complets
   ...
   - **Exemple 5 : Téléchargement de Photos (Nouveau)**
6. Limitations
7. Dépannage
8. Contribuer
9. Licence
...
Supprimez un message avec `deleteMessage`.
...
Utilisez `getFile()` pour obtenir les informations d'un fichier à partir de son `file_id`.
...
Téléchargez un fichier avec `downFile()` en utilisant le chemin obtenu de `getFile()`.

```php
// Télécharger et sauvegarder dans un fichier
$success = $bot->downFile($file_path, 'fichier_local.jpg');

// Ou récupérer le contenu directement
$file_content = $bot->downFile($file_path);
if ($file_content) {
    file_put_contents('fichier_local.jpg', $file_content);
}
```

#### Télécharger des photos

Utilisez `downloadPhoto()` pour télécharger directement une photo avec son `file_id`.

```php
$success = $bot->downloadPhoto('FILE_ID_PHOTO', 'photo_sauvegardee.jpg');
if ($success) {
...
Récupérez automatiquement le `file_id` de la photo de meilleure qualité depuis les mises à jour.

```php
$updates = $bot->getUpdates();
$photo_file_id = $bot->getHighestQualityPhotoFileId($updates);

if ($photo_file_id) {
    $bot->downloadPhoto($photo_file_id, 'photo_utilisateur.jpg');
    echo "Photo extraite et téléchargée !\n";
}
```
...
// Méthode 1 : Téléchargement direct avec file_id connu
$file_id = "AgACAgQAAxkBAAICPmji9eITS7XrYlSuQ4wWlT9Rcs61AAIyyzEbQIAZUyRuIQqS0SinAQADAgADeQADNgQ";
$success = $bot->downloadPhoto($file_id, "photo_telechargee.jpg");

if ($success) {
    echo "Photo téléchargée avec succès !\n";
}

// Méthode 2 : Traitement des mises à jour pour trouver des photos
$updates = $bot->getUpdates();
foreach ($updates['result'] as $update) {
    if (isset($update['message']['photo'])) {
        $photo_file_id = $bot->getHighestQualityPhotoFileId(['result' => [$update]]);
        
        if ($photo_file_id) {
            $filename = "photo_" . time() . ".jpg";
            $bot->downloadPhoto($photo_file_id, $filename);
            echo "Photo sauvegardée : $filename\n";
            
            // Répondre à l'utilisateur
            $chatId = $update['message']['chat']['id'];
            $bot->sendMessage($chatId, "J'ai bien reçu votre photo !");
        }
    }
}
```
...
6. **Téléchargement de fichiers** :
   - Les fichiers sont téléchargés en mémoire, ce qui peut poser problème pour les fichiers volumineux.
...
- **Téléchargement de fichiers échoue** :
  - Vérifiez que le `file_id` est valide et n'a pas expiré.
  - Assurez-vous que le bot a accès au fichier.
